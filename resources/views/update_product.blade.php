...
@section('section')
    ...
    <br> --}}
    <h3>{{$product->name}}</h3>
    <img id="product_image" src="../storage/ProductImages/{{$product->product_image}}" alt="">
    <form method="POST"  action="{{Route('admin.updateproduct', ['id' => $product->id])}}" enctype="multipart/form-data">
        @csrf
        @method('PATCH')
        <div class="form-group">
            <label for="exampleFormControlFile1">Image</label>
            <input type="file" class="form-control-file" id="exampleFormControlFile1" name="product_image" alt="">
        </div>
        ...
        <div class="form-group">
            ...
            <input type="number" class="form-control" id="exampleFormControlInput1" name="stock" value="{{$product->quantity}}">
        </div>
        @if ($errors->any())
